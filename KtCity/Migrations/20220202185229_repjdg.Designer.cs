// <auto-generated />
using System;
using KtCity.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace KtCity.Migrations
{
    [DbContext(typeof(KcDbContext))]
    [Migration("20220202185229_repjdg")]
    partial class repjdg
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.13")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("KtCity.Models.About", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EmailString")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RekviziteStrng")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Tag")
                        .HasColumnType("bit");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("About");
                });

            modelBuilder.Entity("KtCity.Models.AboutAtachFiles", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Anotation")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FileName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Linksid")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("Linksid");

                    b.ToTable("AboutAtachFiles");
                });

            modelBuilder.Entity("KtCity.Models.Admin", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Login")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Password")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Admin");
                });

            modelBuilder.Entity("KtCity.Models.AtachFiles", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("Commandsid")
                        .HasColumnType("int");

                    b.Property<string>("FileName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Newsid")
                        .HasColumnType("int");

                    b.Property<int?>("Vacancyid")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("Commandsid");

                    b.HasIndex("Newsid");

                    b.HasIndex("Vacancyid");

                    b.ToTable("AtachFiles");
                });

            modelBuilder.Entity("KtCity.Models.Cods", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("CodeName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CodeNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PositionInList")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.ToTable("Codes");
                });

            modelBuilder.Entity("KtCity.Models.Commands", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PostedDate")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Commands");
                });

            modelBuilder.Entity("KtCity.Models.Email", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("Aboutid")
                        .HasColumnType("int");

                    b.Property<string>("EmailText")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.HasIndex("Aboutid");

                    b.ToTable("Emails");
                });

            modelBuilder.Entity("KtCity.Models.Galery", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Cover")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Galeries");
                });

            modelBuilder.Entity("KtCity.Models.GaleryImgs", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("Galeryid")
                        .HasColumnType("int");

                    b.Property<string>("ImgName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.HasIndex("Galeryid");

                    b.ToTable("GaleryImages");
                });

            modelBuilder.Entity("KtCity.Models.Judge", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Biography")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Birthday")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Chariman")
                        .HasColumnType("bit");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Positionid")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("Positionid");

                    b.ToTable("Judges");
                });

            modelBuilder.Entity("KtCity.Models.News", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PostedDate")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("News");
                });

            modelBuilder.Entity("KtCity.Models.Palats", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Rang")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.ToTable("Palats");
                });

            modelBuilder.Entity("KtCity.Models.Position", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Rang")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.ToTable("Positions");
                });

            modelBuilder.Entity("KtCity.Models.Statistic", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("StaticText")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Statistic");
                });

            modelBuilder.Entity("KtCity.Models.Struct", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("StaticText")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Structura");
                });

            modelBuilder.Entity("KtCity.Models.Vacancy", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PostedDate")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Vacancies");
                });

            modelBuilder.Entity("KtCity.Models.Worker", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Biography")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Birthday")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Emal")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Judgeid")
                        .HasColumnType("int");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("Positionid")
                        .HasColumnType("int");

                    b.HasKey("id");

                    b.HasIndex("Judgeid");

                    b.HasIndex("Positionid");

                    b.ToTable("Workers");
                });

            modelBuilder.Entity("KtCity.Models.links", b =>
                {
                    b.Property<int>("id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Link")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PositinInLink")
                        .HasColumnType("int");

                    b.Property<string>("Text")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("id");

                    b.ToTable("Links");
                });

            modelBuilder.Entity("KtCity.Models.AboutAtachFiles", b =>
                {
                    b.HasOne("KtCity.Models.links", "Links")
                        .WithMany()
                        .HasForeignKey("Linksid");

                    b.Navigation("Links");
                });

            modelBuilder.Entity("KtCity.Models.AtachFiles", b =>
                {
                    b.HasOne("KtCity.Models.Commands", null)
                        .WithMany("AttachFiles")
                        .HasForeignKey("Commandsid");

                    b.HasOne("KtCity.Models.News", null)
                        .WithMany("AttachFiles")
                        .HasForeignKey("Newsid");

                    b.HasOne("KtCity.Models.Vacancy", null)
                        .WithMany("AttachFiles")
                        .HasForeignKey("Vacancyid");
                });

            modelBuilder.Entity("KtCity.Models.Email", b =>
                {
                    b.HasOne("KtCity.Models.About", "About")
                        .WithMany()
                        .HasForeignKey("Aboutid");

                    b.Navigation("About");
                });

            modelBuilder.Entity("KtCity.Models.GaleryImgs", b =>
                {
                    b.HasOne("KtCity.Models.Galery", "Galery")
                        .WithMany("Images")
                        .HasForeignKey("Galeryid");

                    b.Navigation("Galery");
                });

            modelBuilder.Entity("KtCity.Models.Judge", b =>
                {
                    b.HasOne("KtCity.Models.Palats", "Position")
                        .WithMany("Judges")
                        .HasForeignKey("Positionid");

                    b.Navigation("Position");
                });

            modelBuilder.Entity("KtCity.Models.Worker", b =>
                {
                    b.HasOne("KtCity.Models.Judge", "Judge")
                        .WithMany("Aparati")
                        .HasForeignKey("Judgeid");

                    b.HasOne("KtCity.Models.Position", "Position")
                        .WithMany("Workers")
                        .HasForeignKey("Positionid");

                    b.Navigation("Judge");

                    b.Navigation("Position");
                });

            modelBuilder.Entity("KtCity.Models.Commands", b =>
                {
                    b.Navigation("AttachFiles");
                });

            modelBuilder.Entity("KtCity.Models.Galery", b =>
                {
                    b.Navigation("Images");
                });

            modelBuilder.Entity("KtCity.Models.Judge", b =>
                {
                    b.Navigation("Aparati");
                });

            modelBuilder.Entity("KtCity.Models.News", b =>
                {
                    b.Navigation("AttachFiles");
                });

            modelBuilder.Entity("KtCity.Models.Palats", b =>
                {
                    b.Navigation("Judges");
                });

            modelBuilder.Entity("KtCity.Models.Position", b =>
                {
                    b.Navigation("Workers");
                });

            modelBuilder.Entity("KtCity.Models.Vacancy", b =>
                {
                    b.Navigation("AttachFiles");
                });
#pragma warning restore 612, 618
        }
    }
}
